...
const SalesView = React.memo(function SalesView() {
  ...
  const handleShortcutAction = useCallback((action: string) => {
    switch (action) {
      ...
      case 'clear-cart': if (itemsRef.current.length > 0) { clearCart(); toast('Carrito vaciado', 'info'); focusSearch(); } break;
  ...
  }, [clearCart, focusSearch, toast, navigate]);
  ...
  const handleApplyDiscount = useCallback(() => {
    ...
    if (val > subtotal) { toast(`El descuento no puede superar el subtotal de ${formatMoney(subtotal)}`, 'warning'); return; }
    setDiscount(val);
  ...
  const handleCheckout = useCallback(async (paymentData: any) => {
    ...
    if (!validation.valid) { setPayError(validation.message || 'Stock insuficiente'); setProcessing(false); throw new Error(validation.message || 'Stock invalid'); }
    try {
      const subtotal = currentItems.reduce((acc, item) => acc + (Number(item.price) || 0) * (Number(item.quantity) || 0), 0);
      ...
      try {
        ...
        if (!shouldQueueOffline) { const msg = 'Error al procesar la venta: ' + message; setPayError(msg); toast(msg, 'error'); throw e; }
  ...
  }, [validateStock, generateCheckoutId, printTicket, clearCart, toast, showSaleComplete, restoreAfterSaleComplete, selectedCustomer, clearCustomer]);

  const handleAddManual = useCallback((e: React.FormEvent) => {
    e.preventDefault();
    if (!manualForm.name || !manualForm.price) return;
    addItem({ id: crypto.randomUUID?.() || 'manual-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8), name: manualForm.name || 'Producto', price: parseFloat(manualForm.price || '0'), quantity: 1, stock: 999999, isManual: true } as any);
  ...
  const openPayModal = useCallback(() => {
    const validation = validateStock();
    if (!validation.valid) { setPayError(validation.message ?? 'Stock insuficiente'); toast(validation.message ?? 'Stock insuficiente', 'warning'); return; }
    setPayError('');
  ...
  return (
    <div className="h-full flex gap-0 overflow-hidden">
      ...
      <div className="flex flex-col rounded-l-lg bg-bg-surface border border-border-subtle" style={{ flexBasis: '45%', maxWidth: '480px' }}>
        ...
        <div className="px-4 py-3 border-b border-border-subtle flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-0.5">
            {heldTickets.length > 0 && (
              <button onClick={() => setShowHeldTickets(!showHeldTickets)} className="size-8 rounded-md text-text-secondary hover:text-text-primary hover:bg-bg-surface-hover transition-colors flex items-center justify-center" aria-label={`Ver tickets en espera (${heldTickets.length})`}>
                <Clock className="size-4" />
              </button>
            )}
            <button onClick={() => { if (hasItems) { holdCurrentTicket(); toast('Ticket en pausa', 'info'); } }} disabled={!hasItems} className="size-8 rounded-md text-text-secondary hover:text-text-primary hover:bg-bg-surface-hover transition-colors disabled:opacity-30 flex items-center justify-center" aria-label="Pausar ticket">
              <Pause className="size-4" />
            </button>
            <button onClick={() => setDiscountOpen(true)} disabled={!hasItems} className="size-8 rounded-md text-text-secondary hover:text-text-primary hover:bg-bg-surface-hover transition-colors disabled:opacity-30 flex items-center justify-center" aria-label="Descuento">
              <Percent className="size-4" />
            </button>
            <button onClick={() => setCustomerModalOpen(true)} className="size-8 rounded-md text-text-secondary hover:text-text-primary hover:bg-bg-surface-hover transition-colors flex items-center justify-center" aria-label="Agregar cliente">
              <UserPlus className="size-4" />
            </button>
            <div className="w-px h-4 bg-border-subtle mx-0.5" />
            <button onClick={() => { if (hasItems) { clearCart(); toast('Carrito vaciado', 'info'); focusSearch(); } }} disabled={!hasItems} className="size-8 rounded-md text-text-tertiary hover:text-danger hover:bg-danger-bg transition-colors disabled:opacity-30 flex items-center justify-center" aria-label="Vaciar carrito">
          ...
          {showHeldTickets && heldTickets.length > 0 && (
            <div className="mb-2 border border-border-subtle rounded-lg bg-bg-surface overflow-hidden">
              <div className="px-3 py-2 border-b border-border-subtle flex items-center justify-between">
                <span className="text-xs font-bold text-text-secondary uppercase tracking-wider">En espera</span>
                <button onClick={() => setShowHeldTickets(false)} className="text-text-tertiary hover:text-text-primary" aria-label="Cerrar"><X className="size-3" /></button>
              </div>
              {heldTickets.map(t => (
                <div key={t.id} className="px-3 py-2.5 flex items-center justify-between border-b border-border-subtle last:border-0">
                  <button onClick={() => { recallTicket(t.id); setShowHeldTickets(false); toast(`Ticket restaurado: ${t.label}`, 'success'); }} className="flex-1 text-left min-w-0">
                  ...
                  <button onClick={() => { removeHeldTicket(t.id); toast('Ticket descartado', 'info'); }} className="text-danger/50 hover:text-danger ml-2 shrink-0" aria-label="Descartar ticket">
        ...
        {discount > 0 && (
          <div className="px-4 py-2 bg-accent-bg border-t border-accent/15 flex items-center justify-between text-sm">
            <span className="font-semibold text-accent-text flex items-center gap-1.5"><Percent className="size-3" />Descuento</span>
        ...
        <div className="px-4 py-3 border-t border-border-subtle space-y-3 shrink-0">
          ...
          <div className="rounded-lg bg-bg-surface border border-border-subtle">
            <div className="p-4 space-y-2">
              ...
              {totals.discount > 0 && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-accent-text font-medium">Descuento</span>
            ...
            <div className="px-4 pb-4 pt-2.5 border-t border-border-subtle flex items-baseline justify-between">
              <span className="text-xs font-bold text-text-tertiary uppercase tracking-widest">Total</span>
          ...
          {!hasItems && (
            <p className="text-sm text-text-tertiary text-center font-medium">Escanee un producto para comenzar</p>
          )}
          ...
          >
            {isProcessing ? (
              ...
              <><Wallet className="size-5" /><span>COBRAR {formatMoney(totals.total)}</span></>
            )}
          </button>
        </div>
      </div>
      ...
      {/* Modals */}
      <Modal open={isManualModalOpen} onClose={() => { setManualModalOpen(false); restoreAfterModal(); }} title="Producto Manual" size="sm">
        <form onSubmit={handleAddManual} className="space-y-4">
          ...
          <Button type="submit" size="lg" className="w-full">Agregar al Carrito</Button>
        </form>
      </Modal>

      <Modal open={isDiscountOpen} onClose={() => { setDiscountOpen(false); restoreAfterModal(); }} title="Descuento" size="sm">
        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-text-tertiary uppercase tracking-wider">Monto a descontar</label>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 font-medium text-text-tertiary text-sm">$</span>
          ...
          <div className="flex gap-2">
            <Button variant="secondary" className="flex-1" onClick={() => { setDiscountOpen(false); restoreAfterModal(); }}>Cancelar</Button>
            <Button className="flex-1" onClick={handleApplyDiscount} disabled={!discountValue || parseFloat(discountValue) <= 0}>Aplicar</Button>
          </div>
          {discount > 0 && (
            <button onClick={() => { setDiscount(0); setDiscountOpen(false); toast('Descuento eliminado', 'info'); restoreAfterModal(); }} className="w-full text-xs font-semibold text-danger hover:bg-danger-bg py-2.5 rounded-lg transition-colors">
      ...
      {cashOpen === false && (
        <div className="fixed inset-0 z-[var(--z-overlay)] flex items-center justify-center bg-black/40">
          <div className="bg-bg-surface rounded-xl shadow-dialog p-8 text-center max-w-sm animate-scale-in">
            ...
            <p className="text-sm text-text-secondary mb-6">Debes abrir caja antes de realizar ventas.</p>
